...
const appUserDataPath = process.env.WEAVE_DESKTOP_USER_DATA ||
  path.join(app.getPath("appData"), appName);
const sharedConnectionUserDataPath =
  process.env.WEAVE_DESKTOP_CONNECTION_USER_DATA ||
  process.env.WEAVE_DESKTOP_USER_DATA ||
  path.join(app.getPath("appData"), "Weave");
const shouldPersistEnvAuthToken = process.argv.includes(
  "--persist-env-auth-token",
);
const devAppIconPath = app.isPackaged
  ? undefined
...
const getRpc = () => {
  if (!desktopRpcConnection) {
    throw new Error("Desktop RPC connection is not initialized.");
  }
...
const testConnection = async (
  ...
  try {
    ...
    const token = Object.hasOwn(input ?? {}, "authToken")
    ...
    const connection = new RpcConnection({
      ...
      initialize: {
        ...
        client: {
          clientAppId: "weave",
          clientInstanceId: `desktop-test_${crypto.randomUUID()}`,
        },
      },
    });
    const response = await connection.request("owner.get")
      .finally(() => connection.close());
    if (
      typeof response.owner?.id !== "string" ||
      typeof response.owner.name !== "string"
    ) {
      return {
        ok: false,
        error: "Connection response did not include a valid owner.",
      };
    }
    return {
      ok: true,
      user: { id: response.owner.id, name: response.owner.name },
    };
  } catch (error) {
    return {
      ok: false,
...
const getChatGPTLoginBroker = () => {
  if (!chatGPTLoginBroker) {
    chatGPTLoginBroker = new ChatGPTLoginBroker({
      startLogin: () => getRpc().request("agent.chatgpt.login.start"),
      completeLogin: (params) =>
        getRpc().request("agent.chatgpt.login.complete", params),
      openExternal,
    });
  }
    ...
    portalSupervisor.subscribe((status) => {
      for (const window of BrowserWindow.getAllWindows()) {
        if (!window.webContents.isDestroyed()) {
          window.webContents.send("portal:status", status);
        }
      }
    });
  }
...
const registerIpcHandlers = () => {
  ...
  ipcMain.handle("connection:save-settings", async (_event, input: unknown) => {
    const settings = await getSettingsStore().saveSettingsAsync(
      parseDesktopConnectionInput(input),
    );
    getRpc().reconnect();
    void getPortalSupervisor().reconcile({ refreshPortalToken: true }).catch(
      (error) => console.error("[portal]", error),
    );
    return settings;
  });
  ipcMain.handle(
    ...
    (_event, input?: unknown) =>
      testConnection(
        input === undefined ? undefined : parseDesktopConnectionInput(input),
      ),
  );
  ...
  ipcMain.handle(
    "shell:open-external",
    (_event, url: string) => openExternal(url),
  );
  ipcMain.handle("chatgpt:connect", () => getChatGPTLoginBroker().connect());
  ipcMain.handle(
    "native-notifications:get-permission-state",
    () => getNativeNotificationPermissionState(),
  );
  ipcMain.handle(
    "native-notifications:request-permission",
    () => requestNativeNotificationPermission(),
  );
  ipcMain.handle(
...
app.whenReady().then(() => {
  ...
  return settingsStore.initialize();
}).then(async () => {
  if (!shouldPersistEnvAuthToken) return;
  const store = getSettingsStore();
  const authToken = store.getAuthToken();
  if (!authToken) {
    throw new Error(
      "--persist-env-auth-token requires WEAVE_OWNER_TOKEN or WEAVE_AUTH_TOKEN.",
    );
  }
  await store.saveSettingsAsync({
    mastraUrl: store.getSettings().mastraUrl,
    authToken,
  });
}).then(() => {
  desktopRpcConnection = new DesktopRpcConnection(getSettingsStore());
  registerIpcHandlers();
  getPortalSupervisor().startMonitoring();
  if (devAppIconPath && process.platform === "darwin") {
