...
const createAsyncEncryption = (): EncryptionProvider => ({
  isEncryptionAvailable: () => true,
  encryptString: () => {
    throw new Error("Synchronous encryption is unavailable.");
  },
  decryptString: () => {
    throw new Error("Synchronous decryption is unavailable.");
  },
  isAsyncEncryptionAvailable: async () => true,
  encryptStringAsync: async (value) => Buffer.from(`rotated:${value}`, "utf8"),
...
const withTempStore = async <T>(callback: (path: string) => T | Promise<T>) => {
...
describe("normalizeMastraUrl", () => {
  ...
  it("adds http and removes trailing slashes", () => {
    expect(normalizeMastraUrl("localhost:4111///")).toBe(
      "http://localhost:4111",
    );
  });

  it("preserves a proxy base path without query or hash", () => {
    expect(normalizeMastraUrl("https://example.com/weave/?debug=true#top"))
      .toBe("https://example.com/weave");
  });

  it("rejects non-http protocols and credentials", () => {
    expect(() => normalizeMastraUrl("file:///tmp/weave")).toThrow(
      /http or https/,
    );
    expect(() => normalizeMastraUrl("https://token@example.com")).toThrow(
      /credentials/,
    );
  });

  it("extracts the exact origin for auth injection", () => {
    expect(getServerOrigin("https://example.com/weave")).toBe(
      "https://example.com",
    );
  });
});
...
describe("parseDesktopConnectionInput", () => {
  it("accepts a server URL with optional token states", () => {
    expect(parseDesktopConnectionInput({ mastraUrl: "http://localhost:4111" }))
      .toEqual({
  ...
  it("rejects malformed IPC payloads", () => {
    ...
    expect(() => parseDesktopConnectionInput({ mastraUrl: 4111 })).toThrow(
      /server URL/,
    );
    expect(() =>
      parseDesktopConnectionInput({
...
describe("ConnectionSettingsStore", () => {
  it("persists server URLs and encrypted auth tokens", () =>
    withTempStore((directory) => {
      ...
      expect(
        firstStore.saveSettings({
          mastraUrl: "localhost:4111",
          authToken: "secret",
        }),
      ).toEqual({
        mastraUrl: "http://localhost:4111",
  ...
  it("recovers and rotates persisted tokens through asynchronous safe storage", async () =>
    await withTempStore(async (directory) => {
      const legacyStore = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createEncryption(),
        env: {},
      });
      legacyStore.saveSettings({
        mastraUrl: "localhost:4111",
        authToken: "secret",
      });
      ...
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createAsyncEncryption(),
        env: {},
      });
      await store.initialize();

      expect(store.getSettings().hasAuthToken).toBe(true);
      expect(store.getAuthToken()).toBe("secret");
    }));

  it("encrypts newly saved tokens through asynchronous safe storage", async () =>
    await withTempStore(async (directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createAsyncEncryption(),
  ...
  it("keeps tokens session-only when safe storage is unavailable", () =>
    withTempStore((directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
  ...
  it("clears stored tokens explicitly", () =>
    withTempStore((directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createEncryption(),
  ...
  it("uses environment defaults before persisted settings exist", () =>
    withTempStore((directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createEncryption(),
  ...
  it("uses owner token environment defaults", () =>
    withTempStore((directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createEncryption(),
  ...
  it("uses a single legacy auth token map entry as a compatibility default", () =>
    withTempStore((directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createEncryption(),
  ...
  it("ignores multi-entry legacy auth token maps", () =>
    withTempStore((directory) => {
      const store = new ConnectionSettingsStore({
        userDataPath: directory,
        encryption: createEncryption(),
