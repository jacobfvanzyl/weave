...
const isSharedClientPackage = (source: string) =>
  sharedClientPackages.some((packageName) =>
...
const sharedClientDependencyResolver = (): Plugin => ({
  ...
  async resolveId(source, importer, options) {
    if (!importer?.startsWith(clientSrc) || !isSharedClientPackage(source)) {
      return null;
    }

    return this.resolve(
      source,
      path.join(desktopSrc, "__weave_client_dependency_anchor.ts"),
      {
        ...options,
        skipSelf: true,
      },
