...
- Physical iPad: build and XCTest passed on the connected iPad Air M3 (31.674 seconds, zero failures). The disposable TLS Host exercised ACP permissions, native terminal typing, Neovim, portrait/landscape rotation and composer keyboard geometry. Evidence: `/tmp/wve65-composition-ipad.xcresult` and `/tmp/wve65-ipad-shell.json`. Exported portrait and composer screenshots were inspected. The fixture connection was forgotten, its Host/tmux stopped, and the previous iPad app artifact was reinstalled and launched.
