...
const buildInput = (mastraUrl: string, authToken: string, clearToken: boolean): ConnectionInput => {
...
const ConnectionForm = ({
  ...
  const [lastResult, setLastResult] = useState<ConnectionTestResult | undefined>();
    ...
    <form className={compact ? 'space-y-5' : 'w-full max-w-md space-y-6'} onSubmit={submit}>
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          Connect this client to the running Weave server.
        </p>
      </div>
