...
const coppermindBlockSuiteStyles = `
  ...
  [data-weave-editor-coppermind] page-editor {
    --coppermind-page-cell-fill: var(--affine-background-primary-color, #fff);
  }
  ...
  [data-weave-editor-coppermind] page-editor affine-note {
    ...
    border-radius: 0;
    background: var(--coppermind-page-cell-fill);
    box-shadow: none;
    transition: border-color 120ms ease;
  }

  :root[data-theme="mocha"] [data-weave-editor-coppermind] page-editor affine-note {
    border-top-color: color-mix(in oklab, var(--ctp-text) 10%, transparent);
    border-right-color: transparent;
    border-bottom-color: transparent;
    border-left-color: transparent;
    background: var(--ctp-base);
  ...
  [data-weave-editor-coppermind] page-editor affine-note :is(.affine-note-mask, .note-background) {
    border-radius: 0 !important;
    background: var(--coppermind-page-cell-fill) !important;
  }
...
const getInkRuns = (sections: CoppermindBlockSuiteSection[]) => {
  ...
  let current: CoppermindBlockSuiteSection[] = [];

  const flush = () => {
    if (current.length > 0) {
      groups.push({
        sections: current,
        stacked: current.length > 1 && current.every(isStackableInkSection),
      });
      current = [];
    }
  };
...
const findStackedInkRunForSection = (
  ...
  const run = findInkRunForSection(sections, sectionId);
  return run?.stacked ? run : undefined;
};

const sameXYWH = (
  a: CoppermindBlockSuiteSectionXYWH,
  b: CoppermindBlockSuiteSectionXYWH,
) => a.every((value, index) => Math.abs(value - b[index]) < 1);

const getStackedInkRunXYWH = (
  group: CoppermindInkStackGroup,
  ...
  return group.sections.map(section => {
    ...
    y += section.height + coppermindCanvasCellGapPx;
    return { section, xywh };
  });
};

const syncInkStackPlacements = (
  doc: Doc,
  sections: CoppermindBlockSuiteSection[],
...
export const CoppermindDocumentEditor = forwardRef<CoppermindDocumentEditorHandle, CoppermindDocumentEditorProps>(({
  ...
  useEffect(() => {
    ...
    const handleInkCellFocus = (event: Event) => {
