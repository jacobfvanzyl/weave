...
- (instancetype)initWithSessionId:(NSString *)sessionId
    ...
    _maxDimension = std::max<NSInteger>(320, std::min<NSInteger>(4096, maxDimension));
    _frameRate = std::max<NSInteger>(1, std::min<NSInteger>(60, frameRate));
    _bitrate = std::max<NSInteger>(500000, bitrate);
...
- (BOOL)startCaptureWithError:(NSError **)outError {
    ...
    WriteDiagnostic([NSString stringWithFormat:@"capture started: backend=native-webrtc window=%@ size=%ldx%ld fps=%ld cursor=false bitrate=%ld",
                                                _windowId, static_cast<long>(_width), static_cast<long>(_height),
                                                static_cast<long>(_frameRate), static_cast<long>(_bitrate)]);
...
- (BOOL)createEncoderWithError:(NSError **)outError {
    ...
    if (status != noErr || !_encoder) {
        if (outError) {
            *outError = [NSError errorWithDomain:@"weave.window-stream-native"
                                            code:8
                                        userInfo:@{NSLocalizedDescriptionKey: [NSString stringWithFormat:@"VideoToolbox encoder creation failed: %d", status]}];
        }
        return NO;
    }
    ...
    int32_t keyframeInterval = static_cast<int32_t>(_frameRate * 2);
    double keyframeIntervalDuration = 2.0;
    int32_t maxFrameDelay = 0;

    VTSessionSetProperty(_encoder, kVTCompressionPropertyKey_RealTime, kCFBooleanTrue);
    VTSessionSetProperty(_encoder, kVTCompressionPropertyKey_AllowFrameReordering, kCFBooleanFalse);
    VTSessionSetProperty(_encoder, kVTCompressionPropertyKey_ProfileLevel, kVTProfileLevel_H264_Baseline_AutoLevel);
...
- (BOOL)handleMessage:(NSDictionary *)message {
    ...
    if ([type isEqualToString:@"session.start"]) {
        ...
        NSInteger frameRate = OptionalInteger(message[@"maxFrameRate"], 60);
        NSInteger maxDimension = OptionalInteger(message[@"maxDimension"], 1920);
        NSString *bitrateEnv = OptionalString([NSProcessInfo processInfo].environment[@"WEAVE_WINDOW_STREAM_BITRATE"]);
        NSInteger bitrate = bitrateEnv ? bitrateEnv.integerValue : 12000000;
        bitrate = OptionalInteger(message[@"bitrate"], bitrate);
