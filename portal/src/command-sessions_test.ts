...
Deno.test("command sessions stream, poll, and retain output offsets", async () => {
  ...
  try {
    ...
    parsePortalToolResult("exec_start", { ok: true, ...started });
    ...
    parsePortalToolResult("exec_poll", { ok: true, ...polled });
  } finally {
...
Deno.test("command sessions accept stdin and can be cancelled", async () => {
  ...
  try {
    ...
    for (
      let attempt = 0;
      attempt < 50 && polled.status === "running";
      attempt += 1
    ) {
    ...
    assertStringIncludes(
      polled.events.map((event) => event.text).join(""),
      "value=hello",
    );
  } finally {
    sessions.clearForTests();
...
Deno.test("oversized command output preserves head and tail and retrieves omitted ranges by offset", async () => {
  ...
  try {
    ...
    const middle = await sessions.poll(
      root,
      started.id,
      omitted.startOffset,
      128,
    ...
    sessions.clearForTests();
    const restored = await new CommandSessionManager(artifactRoot).poll(
      root,
      started.id,
      omitted.startOffset,
      128,
...
Deno.test("command sessions enforce timeout, cancellation, and PTY execution", async () => {
  ...
  try {
    ...
    assertStringIncludes(
      pty.events.map((event) => event.text).join(""),
      "pty-ok",
    );
  } finally {
    sessions.clearForTests();
