...
const createRemoteBackedRepo = async (prefix: string) => {
  ...
  const remote = await Deno.realPath(
    await Deno.makeTempDir({ prefix: `${prefix}-remote-` }),
  );
  ...
  const peerParent = await Deno.realPath(
    await Deno.makeTempDir({ prefix: `${prefix}-peer-` }),
  );
...
Deno.test("Git module parses status, diff, and log output", async () => {
  ...
  try {
    ...
    parsePortalToolResult("portal.git.status", { ok: true, ...status });
    assertEquals(status.branch, "main");
    assert(
      status.entries.some((entry) =>
        entry.path === "README.md" && entry.unstaged === "M"
      ),
    );
    assert(
      status.entries.some((entry) =>
        entry.path === "notes.txt" && entry.kind === "untracked"
      ),
    );

    const diff = await getGitDiff(repo, {});
    parsePortalToolResult("portal.git.diff", { ok: true, ...diff });
    assertStringIncludes(diff.diff, "+changed");

    const log = await getGitLog(repo, { limit: 1 });
    parsePortalToolResult("portal.git.log", { ok: true, ...log });
...
Deno.test("Git module creates, lists, and validates worktrees without Worktrunk", async () => {
  ...
  try {
    const worktree = await createGitWorktree(repo, {
      mode: "newBranch",
      branch: "feature/git-module",
      name: "git-module",
    });
    parsePortalToolResult("portal.git.worktree.create", { ok: true, worktree });
    worktreePath = worktree.path;
    assert(worktreePath);
    assertEquals(worktree.branch, "feature/git-module");

    const worktrees = await listGitWorktrees(repo);
    parsePortalToolResult("portal.git.worktree.list", { ok: true, worktrees });
    assert(
      worktrees.some((item) =>
        item.path === worktreePath && item.branch === "feature/git-module"
      ),
    );
    ...
    if (worktreePath) {
      await runGit(repo, ["worktree", "remove", worktreePath, "--force"]).catch(
        () => undefined,
      );
    }
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
...
Deno.test("Git module removes worktrees while preserving branches by default", async () => {
  ...
  try {
    ...
    const result = await removeGitWorktree(repo, {
      path: worktreePath,
      defaultBranch: "main",
    });
    ...
    assertEquals(
      await runGitOutput(repo, ["branch", "--list", "feature/preserve"]),
      "feature/preserve",
    );
  } finally {
    if (worktreePath) {
    ...
    await runGit(repo, ["branch", "-D", "feature/preserve"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
    if (worktreePath) {
...
Deno.test("Git module deletes a merged local branch only after removing its worktree", async () => {
  ...
  try {
    const worktree = await createGitWorktree(repo, {
      mode: "newBranch",
      branch: "feature/delete-merged",
      name: "delete-merged",
    });
    worktreePath = worktree.path;
    assert(worktreePath);

    const preview = await inspectGitBranchCleanup(repo, {
      path: worktreePath,
      defaultBranch: "main",
    });
    ...
    assertEquals(
      await runGitOutput(repo, ["branch", "--list", "feature/delete-merged"]),
      "",
    );
  } finally {
    if (worktreePath) {
    ...
    await runGit(repo, ["branch", "-D", "feature/delete-merged"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
    if (worktreePath) {
...
Deno.test("Git module keeps unmerged local branches when branch deletion is requested", async () => {
  ...
  try {
    ...
    assertEquals(
      await runGitOutput(repo, ["branch", "--list", "feature/unmerged"]),
      "feature/unmerged",
    );
  } finally {
    if (worktreePath) {
    ...
    await runGit(repo, ["branch", "-D", "feature/unmerged"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
    if (worktreePath) {
...
Deno.test("Git module deletes a no-upstream branch when same-name remote contains it", async () => {
  const { repo, remote, peerParent } = await createRemoteBackedRepo(
    "weave-git-worktree-remove-same-remote-",
  );
  let worktreePath: string | undefined;
  try {
    const worktree = await createGitWorktree(repo, {
      mode: "newBranch",
      branch: "feature/pushed-pr",
      name: "pushed-pr",
    });
    worktreePath = worktree.path;
    assert(worktreePath);
    ...
    await runGit(worktreePath, [
      "push",
      "origin",
      "HEAD:refs/heads/feature/pushed-pr",
    ]);
    await runGit(repo, [
      "fetch",
      "origin",
      "feature/pushed-pr:refs/remotes/origin/feature/pushed-pr",
    ]);
    ...
    assertEquals(
      await runGitOutput(repo, ["branch", "--list", "feature/pushed-pr"]),
      "",
    );
  } finally {
    if (worktreePath) {
    ...
    await runGit(repo, ["branch", "-D", "feature/pushed-pr"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
...
Deno.test("Git module reports not pushed when same-name remote is behind the local branch", async () => {
  const { repo, remote, peerParent } = await createRemoteBackedRepo(
    "weave-git-worktree-remove-not-pushed-",
  );
  let worktreePath: string | undefined;
  try {
    const worktree = await createGitWorktree(repo, {
    ...
    await runGit(worktreePath, [
      "push",
      "origin",
      "HEAD:refs/heads/feature/not-pushed",
    ]);
    await runGit(repo, [
      "fetch",
      "origin",
      "feature/not-pushed:refs/remotes/origin/feature/not-pushed",
    ]);
    await commitFile(
      worktreePath,
      "local-only.txt",
    ...
    await runGit(repo, ["branch", "-D", "feature/not-pushed"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
    await Deno.remove(remote, { recursive: true }).catch(() => undefined);
...
Deno.test("Git module does not guess between ambiguous same-name remote branches", async () => {
  ...
  const firstRemote = await Deno.realPath(
    await Deno.makeTempDir({ prefix: "weave-git-first-remote-" }),
  );
  const secondRemote = await Deno.realPath(
    await Deno.makeTempDir({ prefix: "weave-git-second-remote-" }),
  );
  let worktreePath: string | undefined;
  try {
    ...
    await runGit(worktreePath, [
      "push",
      "first",
      "HEAD:refs/heads/feature/ambiguous",
    ]);
    await runGit(worktreePath, [
      "push",
      "second",
    ...
    await runGit(repo, [
      "fetch",
      "first",
      "feature/ambiguous:refs/remotes/first/feature/ambiguous",
    ]);
    await runGit(repo, [
      "fetch",
      "second",
      "feature/ambiguous:refs/remotes/second/feature/ambiguous",
    ]);
    ...
    assertStringIncludes(
      preview.error ?? "",
      "multiple remote branches match feature/ambiguous",
    );
  } finally {
    if (worktreePath) {
      await runGit(repo, ["worktree", "remove", worktreePath, "--force"]).catch(
    ...
    await runGit(repo, ["branch", "-D", "feature/ambiguous"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
...
Deno.test("Git module requires explicit force to remove dirty worktrees", async () => {
  ...
  try {
    ...
    assertEquals(result.branchCleanup.status, "not_requested");
    assertEquals(
      await runGitOutput(repo, ["branch", "--list", "feature/dirty"]),
      "feature/dirty",
    );
  } finally {
    if (worktreePath) {
    ...
    await runGit(repo, ["branch", "-D", "feature/dirty"]).catch(() =>
      undefined
    );
    await Deno.remove(repo, { recursive: true }).catch(() => undefined);
    if (worktreePath) {
...
Deno.test("Git module fetches upstream and parses ahead behind status", async () => {
  const { repo, remote, peerParent, peer } = await createRemoteBackedRepo(
    "weave-git-upstream-",
  );
  try {
