...
export type TerminalStartedEvent = Extract<TerminalHostEvent, { type: 'started' }>;
...
export type PortalTerminalHostOptions = {
  ...
  /** @deprecated replay is now captured from tmux on attach instead of buffered in Portal. */
...
const preferredTmuxDefaultTerminal = 'tmux-256color';
const fallbackTmuxDefaultTerminal = 'screen-256color';
const tmuxTerminalNamePattern = /^[A-Za-z0-9][A-Za-z0-9_.+-]*$/;
const tmuxDefaultTerminalCache = new Map<string, Promise<string>>();

const normalizeTmuxTerminalName = (value: string | undefined) => {
  const trimmed = value?.trim();
  return trimmed && tmuxTerminalNamePattern.test(trimmed) ? trimmed : undefined;
};

export const resolveTmuxDefaultTerminal = async (env: Record<string, string | undefined> = Deno.env.toObject()) => {
  const explicit = normalizeTmuxTerminalName(env.WEAVE_PORTAL_TMUX_TERM);
  if (explicit) return explicit;

  const processEnv = getTerminalProcessEnv(env);
  const cacheKey = processEnv.PATH ?? '';
  let cached = tmuxDefaultTerminalCache.get(cacheKey);
  if (!cached) {
    try {
      cached = new Deno.Command('infocmp', {
        args: [preferredTmuxDefaultTerminal],
        env: processEnv,
        clearEnv: true,
        stdout: 'null',
        stderr: 'null',
      }).output()
        .then((output) => output.success ? preferredTmuxDefaultTerminal : fallbackTmuxDefaultTerminal)
        .catch(() => fallbackTmuxDefaultTerminal);
    } catch {
      cached = Promise.resolve(fallbackTmuxDefaultTerminal);
    }
    tmuxDefaultTerminalCache.set(cacheKey, cached);
  }
  return await cached;
};
...
export type PortalTmuxControlClient = {
  ...
  continueOutput: (paneId: string) => Promise<void>;
  close: () => void;
};
...
const tmuxConfigVersion = 'weave-tmux-config-v4';
const tmuxConfigVersionOption = '@weave_config_version';
const buildTmuxConfig = (defaultTerminal: string) => `# Generated by Weave. Do not edit.
...
export const parseTmuxControlNotification = (
  ...
  if (line.startsWith('%pause ')) {
    const [, paneId = ''] = line.split(/\s+/, 2);
    return paneId ? { type: 'pause', paneId } : { type: 'other' };
  }

  if (line.startsWith('%continue ')) {
    const [, paneId = ''] = line.split(/\s+/, 2);
    return paneId ? { type: 'continue', paneId } : { type: 'other' };
  }
...
class TmuxControlModeClient implements PortalTmuxControlClient {
  ...
  async continueOutput(paneId: string) {
    await this.sendCommand(['refresh-client', '-A', `${paneId}:continue`]);
  }
  ...
  private handleLine(line: string) {
    ...
    if (notification.type === 'pause') {
      this.options.handlers.onPause(notification.paneId);
      return;
    }
    if (notification.type === 'continue') {
      this.options.handlers.onContinue(notification.paneId);
      return;
    }
...
export class TmuxTerminalController implements PortalTmuxController {
  ...
  private defaultTerminalPromise?: Promise<string>;
  ...
  async openControlClient(handlers: PortalTmuxControlClientHandlers) {
    ...
    const client = this.controlClientFactory({
      args: [...this.tmuxBaseArgs(), '-C', 'attach-session', '-f', 'pause-after=1', '-t', this.sessionName],
      env,
      handlers,
  ...
  private async applySessionHardening(cwd: string, env: Record<string, string>) {
    ...
    const commands: Array<{ args: string[]; okCodes?: number[] }> = [
      ...
      { args: ['set-option', '-g', 'default-terminal', defaultTerminal] },
      { args: ['set-option', '-ga', 'terminal-overrides', `,${defaultTerminal}:Tc`] },
  ...
  private async ensureConfigFile() {
    ...
    await Deno.writeTextFile(this.configPath, buildTmuxConfig(defaultTerminal), { mode: 0o600 });
    await Deno.chmod(this.configPath, 0o600).catch(() => undefined);
    this.configWritten = true;
  }

  private getDefaultTerminal() {
    this.defaultTerminalPromise ??= resolveTmuxDefaultTerminal(this.env);
    return this.defaultTerminalPromise;
  }
...
export class PortalTerminalHost {
  ...
  private async create(input: TerminalTargetInput & { requestId?: string }, send: (event: TerminalHostEvent) => void) {
    ...
    const window = await this.tmux.createWindow(target, {
      env: await this.getWeaveEnv(target),
  ...
  private async start(input: TerminalStartInput, clientId: string, send: (event: TerminalHostEvent) => void) {
    ...
    try {
      ...
      if (existing && !existing.exited) {
        ...
        const replay = await this.captureReplay(existing.window) ?? '';
        ...
        this.sendReplay(existing, replay, send);
        return;
      }
      ...
      const window = await this.tmux.ensureWindow(target, {
        terminalId: normalizedInput.terminalId,
        env: await this.getWeaveEnv({ ...target, terminalId: normalizedInput.terminalId }),
        shell: getDefaultShell(this.env),
      });
      ...
      this.sendReplay(session, replay, send);
    } catch (error) {
      send({
        type: 'error',
        terminalId: normalizedInput.terminalId,
  ...
  private async getWeaveEnv(input: ResolvedTerminalTarget & { terminalId?: string }) {
    const weaveEnv: Record<string, string> = {
      TERM: await this.getDefaultTerminal(),
      COLORTERM: 'truecolor',
      PROMPT_EOL_MARK: '',
  ...
  private sendReplay(session: TerminalSession, data: string, send: (event: TerminalHostEvent) => void) {
    if (!data) return;
    send({
      type: 'replay',
      terminalId: session.terminalId,
  ...
  private async ensureControlClient() {
    ...
    this.controlClient = await this.tmux.openControlClient({
      ...
      onPause: (paneId) => this.handleControlPause(paneId),
      onContinue: (paneId) => this.handleControlContinue(paneId),
  ...
  private handleControlPause(paneId: string) {
    const terminalId = this.terminalIdsByPaneId.get(paneId);
    const session = terminalId ? this.sessions.get(terminalId) : undefined;
    if (session && !session.exited) this.flushOutput(session);

    void this.controlClient?.continueOutput(paneId).catch((error) => {
      this.handleControlError(error instanceof Error ? error : new Error(String(error)));
    });
  }

  private handleControlContinue(paneId: string) {
    this.debug('host.control.continue', { paneId });
  }
