...
await writeFile(config, JSON.stringify({ listen: { hostname: '127.0.0.1', port }, displayName: 'Desktop acceptance', allowedOrigins: ['weave://app'], stateDirectory: join(root, 'state'), workspaces: [{ workspaceId: 'desktop', name: 'Desktop acceptance', path: workspace }], agents: [{ agentId: 'fixture', name: 'Acceptance', command: provider.command, args: provider.args ?? [], env: {} }] }), { mode: 0o600 });
...
try {
  ...
  for (const phase of (providerConfig ? ['pair'] : ['pair', 'reconnect'])) {
    await writeFile(join(root, 'input.json'), JSON.stringify({ hostUrl: `ws://127.0.0.1:${port}`, ...(phase === 'pair' ? { pairingToken: token } : {}), workspaceName: 'Desktop acceptance', permission: !providerConfig }), { mode: 0o600 });
  ...
  await host.exited;
  clearTimeout(timer);
  // Only the dedicated acceptance tmux server and one-time token file.
  Bun.spawnSync(['tmux', '-S', join(root, 'state/terminal/tmux.sock'), 'kill-server'], { stdout: 'ignore', stderr: 'ignore' });
