...
it('renders independent terminal and agent selection, compact groups, and non-destructive view closure', async () => {
  ...
  await user.click(screen.getByRole('button', { name: 'Actions for terminal workspace Workspace 2' }));
  await user.click(screen.getByRole('menuitem', { name: 'Rename terminal workspace…' }));
  await user.clear(screen.getByRole('textbox', { name: 'Workspace name' }));
  expect(screen.getByRole('button', { name: 'Save name' })).toBeDisabled();
  await user.type(screen.getByRole('textbox', { name: 'Workspace name' }), 'Verification');
  await user.click(screen.getByRole('button', { name: 'Save name' }));
  await waitFor(() => expect(screen.queryByRole('dialog', { name: 'Name terminal workspace' })).not.toBeInTheDocument());
  expect(screen.getByLabelText('Active terminal context')).toHaveTextContent('Verification');
  expect(controller!.model.selectedThreadId).toBe('two:thread');
  expect(rendered.container.querySelector('[data-context-id]')).toBe(root);
  await user.click(screen.getByRole('button', { name: 'Actions for terminal workspace Verification' }));
  await user.click(screen.getByRole('menuitem', { name: 'Move up' }));
  expect(within(root).getAllByRole('button', { name: /^(Workspace 1|Verification)$/ }).map((button) => button.textContent)).toEqual(['Verification', 'Workspace 1']);
  await user.click(screen.getByRole('button', { name: 'Use running terminal…' }));
  await waitFor(() => expect(screen.getByRole('button', { name: 'Use terminal Shell (terminal)' })).toBeInTheDocument());
  // The Host is authoritative even when the dialog listed this session earlier.
  one.listTerminals.mockResolvedValueOnce({ terminals: [] });
  await user.click(screen.getByRole('button', { name: 'Use terminal Shell (terminal)' }));
  await waitFor(() => expect(within(screen.getByRole('dialog', { name: 'Use a running terminal' })).getByRole('alert')).toHaveTextContent('no longer running'));
  expect(one.createTerminal).toHaveBeenCalledTimes(1);
  await user.click(screen.getByRole('button', { name: 'Use terminal Shell (terminal)' }));
  await waitFor(() => expect(screen.queryByRole('dialog', { name: 'Use a running terminal' })).not.toBeInTheDocument());
  await waitFor(() => expect(screen.getByRole('textbox', { name: 'Terminal input' })).toHaveValue('ready'));
  expect(one.createTerminal).toHaveBeenCalledTimes(1);
  expect(one.closeTerminal).not.toHaveBeenCalled();
  expect(controller!.model.selectedThreadId).toBe('two:thread');
  await user.click(screen.getByRole('button', { name: 'Close workspace view Verification' }));
