...
function TerminalSurface({ controller, reference, node, focusRequest, maximized, focused }: { focused: boolean; maximized: boolean; focusRequest?: string; controller: AlphaController; reference: WorkspaceReference; node: Extract<TerminalLayoutNode, { kind: 'terminal' }> }) {
  ...
  // Reserve the focus border below the title rail. Round only the border; the terminal surface remains square.
  return <section className='flex min-h-0 min-w-0 flex-1 flex-col' data-focused={focused || undefined} aria-label={`Terminal pane ${node.paneId}`} data-terminal-id={node.terminalId ?? undefined} onFocusCapture={(event) => { if ((event.target as HTMLElement).closest('[data-slot="native-terminal"]')) controller.workspaceActions?.focus(reference, node.paneId); }}>
    {!available || connecting ? <TerminalPaneSkeleton framed /> : <>
    <header data-slot='terminal-top-rail' className='flex h-[var(--rail-height)] shrink-0 items-center gap-1 border-b bg-title-bar px-2'>
    ...
    <div data-slot='terminal-focus-border' className={cn('flex min-h-0 min-w-0 flex-1 flex-col rounded-[calc(var(--radius-sm)+2px)] border', focused ? 'border-terminal-focus' : 'border-transparent')}>
    ...
    {model.attachmentId ? <TerminalView focusRequest={focusRequest} output={model.output} readOnly={model.attachmentMode === 'observe' || !available} onInput={(data) => void perform(() => actions.input(data))} onResize={(cols, rows) => void perform(() => actions.resize(cols, rows))} /> : <TerminalPaneSkeleton />}
    </div>
    </>}
  </section>;
}
  ...
  return <main className='flex min-h-0 min-w-0 flex-1 flex-col' aria-label='Terminal workspace'>
    ...
    {state.presentation.openWorkspaces.map((reference) => {
      ...
      return <WorkspacePaneLayout key={key} layout={saved.layout} active={active} maximized={maximized}
        ...
        renderPane={(node, visible) => <TerminalSurface controller={controller} reference={reference} node={node} focused={visible && (maximized ?? focused) === node.paneId} maximized={maximized === node.paneId} focusRequest={visible && (maximized ?? focused) === node.paneId ? inputFocusRequest === undefined ? `${key}:${node.paneId}:${maximized ?? ''}` : inputFocusRequest === null ? undefined : inputFocusRequest.workspaceKey === key && inputFocusRequest.paneId === node.paneId ? `sidebar:${inputFocusRequest.token}` : `${key}:${node.paneId}:${maximized ?? ''}` : undefined} />} />;
    })}
