...
const commandText = async (
  ...
  const stdout = options.stdout === "inherit" || options.stdout === "null"
    ? ""
    : textDecoder.decode(output.stdout);
  const stderr = options.stderr === "inherit" || options.stderr === "null"
    ? ""
...
const git = (cwd: string, args: string[], env?: Record<string, string>) =>
  commandText("git", args, { cwd, env });

const requiredEnv = (name: string) => {
  const value = Deno.env.get(name)?.trim();
  if (!value) {
    throw new Error(
      `${name} is required. Copy server/.env.deploy.example to server/.env.deploy.`,
    );
  }
  return value;
};
...
const lastGoodRef = () =>
  optionalEnv("WEAVE_LAST_GOOD_REF", defaultLastGoodRef);

const assertSafeGitRef = (ref: string) => {
  if (
    !/^refs\/heads\/[A-Za-z0-9._\/-]+$/.test(ref) || ref.includes("..") ||
    ref.endsWith("/")
  ) {
...
export const isSuspiciousSnapshotPath = (path: string) => {
  ...
  return /^(id_rsa|id_dsa|id_ecdsa|id_ed25519|credentials|secrets?)$/i.test(
    name,
  );
};
...
export const createServerSnapshot = async (
  ...
  if (conflicts) {
    throw new Error(`Resolve merge conflicts before deploying:\n${conflicts}`);
  }
  ...
  try {
    ...
    await git(cwd, ["add", "-A", "--", ...serverSnapshotPaths], env);
    ...
    if (suspicious.length) {
      throw new Error(
        `Refusing to snapshot possible secrets:\n${
...
export const getRemoteRef = async (
  cwd: string,
  remote: string,
  ref: string,
) => {
...
const apiJson = async <T>(
  ...
  if (!response.ok) {
    const safeBody = config.token
      ? body.replaceAll(config.token, "[REDACTED]")
      : body;
    throw new Error(
      `Dokploy ${path} returned ${response.status}: ${safeBody.slice(0, 2000)}`,
    );
  }
...
export const listDeployments = (config: DokployConfig) =>
  apiJson<DeploymentRecord[]>(
    config,
    `deployment.allByCompose?composeId=${encodeURIComponent(config.composeId)}`,
  );
...
const readDeploymentLogs = async (
  ...
  const response = await apiJson<unknown>(
    config,
    `deployment.readLogs?deploymentId=${
      encodeURIComponent(deployment.deploymentId)
    }&tail=500`,
  ).catch((error) => ({
  ...
  if (target && logPath && /^\/[A-Za-z0-9_./-]+$/.test(logPath)) {
    const sshLogs = await commandText("ssh", [
      target,
      "tail",
      "-n",
      "500",
...
export const triggerAndWaitForDeployment = async (
  ...
  const before = new Set(
    (await listDeployments(config)).map((item) => item.deploymentId),
  );
  ...
  while (Date.now() < deadline) {
    ...
    deployment = deployments.find((item) =>
      item.title === title && !before.has(item.deploymentId)
    ) ??
      deployments.find((item) =>
        !before.has(item.deploymentId)
      );
    if (
    ...
    throw new Error(
      `Dokploy deployment ${deployment.status}: ${
        deployment.errorMessage ?? "no error message"
      }\n${logs}`,
    );
  }
  throw new Error(
    `Timed out waiting for Dokploy deployment${
...
export const verifyRemoteHealth = async (timeoutMs = 90000) => {
  ...
  throw new Error(
    `Remote health verification failed: ${
...
export const verifyRemoteRpc = async (serverUrl: string) => {
  ...
  url.search = "";
  const token = Deno.env.get("WEAVE_REMOTE_OWNER_TOKEN")?.trim() ??
    Deno.env.get("WEAVE_OWNER_TOKEN")?.trim();
  if (!token) {
    throw new Error(
      "WEAVE_REMOTE_OWNER_TOKEN is required for authenticated RPC deployment verification",
    );
  }
  await new Promise<void>((resolve, reject) => {
    ...
    socket.onclose = (event) => {
      if (event.code !== 1000) {
        finish(new Error(`${url} RPC probe closed (${event.code})`));
      }
    };
  });
};
...
const prepare = async () => {
  ...
  console.info(
    `Snapshot includes ${snapshot.files.length} changed server build input(s).`,
  );
  console.info(
...
const deploy = async () => {
  ...
  const deployment = await triggerAndWaitForDeployment(
    config,
    title,
    `Source snapshot ${snapshot.commit}`,
  );
...
const status = async () => {
  ...
  console.info(
    `Deploy ref: ${current ?? "missing"}\nLast good: ${lastGood ?? "missing"}`,
  );
  const latest = deployments[0];
  console.info(
    `Dokploy: ${
      latest
        ? `${latest.status ?? "unknown"} ${latest.title ?? latest.deploymentId}`
        : "no deployments"
    }`,
  );
  try {
    ...
    console.info(
      `Health: unavailable (${error instanceof Error ? error.message : error})`,
    );
  }
};
...
const rollback = async () => {
  ...
  if (!target) {
    throw new Error(
      `Remote ${lastGoodRef()} does not exist; no successful snapshot has been recorded.`,
    );
  }
  ...
  await triggerAndWaitForDeployment(
    config,
    title,
    `Rollback application code to ${target}; database is unchanged.`,
  );
  await verifyRemoteHealth();
  console.info(
    `Rolled application code back to ${target}. Database migrations were not rolled back.`,
  );
};

const shellQuote = (value: string) => `'${value.replaceAll("'", "'\\''")}'`;

const sshText = (target: string, script: string) =>
  commandText("ssh", [target, "bash", "-lc", shellQuote(script)]);

const findRemoteContainer = async (
  target: string,
  appName: string,
  service: string,
  ...
  if (!id) {
    throw new Error(
      `Could not find remote ${service} container for Compose project ${appName}.`,
    );
  }
  return id;
};
...
const localServerIsRunning = async () => {
  try {
    const response = await fetch("http://127.0.0.1:4111/health", {
...
const streamPostgresCutover = async (
  target: string,
  ...
  const restoreScript =
    `docker exec -i ${
      shellQuote(remotePostgres)
    } pg_restore -U supabase_admin -d weave ` +
  ...
  if (!restoreStatus.success) {
    throw new Error(
      `Remote pg_restore failed with exit code ${restoreStatus.code}.`,
    );
  }
};
...
const runObjectCopy = async () => {
  ...
  if (!output.success) {
    throw new Error(`Garage object copy failed with exit code ${output.code}.`);
  }
};
...
const cutover = async () => {
  ...
  const localPostgres = optionalEnv(
    "WEAVE_LOCAL_POSTGRES_CONTAINER",
    "weave-dbos-postgres",
  );
  if (await localServerIsRunning()) {
    throw new Error(
      "Stop the local Weave server on port 4111 before starting cutover.",
    );
  }
  ...
  const remoteServer = await findRemoteContainer(
    target,
    appName,
    "weave-server",
  );
  const remoteMigrate = await findRemoteContainer(
    target,
    appName,
    "weave-migrate",
    true,
  ...
  console.info(
    `Stopping remote server ${
      remoteServer.slice(0, 12)
    } and streaming Postgres schemas...`,
  );
  ...
  try {
    ...
    await sshText(target, `docker start ${shellQuote(remoteServer)}`).catch(
      () => undefined,
    );
    throw error;
  }
  ...
  const remoteSummary = await sshText(
    ...
    `docker exec ${
      shellQuote(remotePostgres)
    } psql -U supabase_admin -d weave -Atc ` +
      shellQuote(
        "select schemaname || ':' || count(*) from pg_tables where schemaname in ('weave','mastra','dbos') group by schemaname order by schemaname;",
      ),
  );
  if (remoteSummary !== localSummary) {
    throw new Error(
      `Postgres schema summary mismatch.\nLocal:\n${localSummary}\nRemote:\n${remoteSummary}`,
    );
  }
  await verifyRemoteHealth();
  ...
  if (localStopConfirmation !== "STOP LOCAL INFRASTRUCTURE") {
    ...
    console.info(
      "Run `deno task infra:down` only after Desktop and Portal have been verified.",
    );
    return;
  }
  await commandText(Deno.execPath(), ["task", "infra:down"], {
    cwd: repoRoot,
    stdout: "inherit",
    stderr: "inherit",
  });
  console.info(
    "Cutover verified. Local infrastructure is stopped; its volumes were preserved.",
  );
};
...
export const main = async (args = Deno.args) => {
  switch (args[0]) {
    ...
    default:
      throw new Error(
        "Usage: dokploy-server.ts <prepare|deploy|status|rollback|cutover>",
      );
  }
};
