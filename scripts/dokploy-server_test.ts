...
const run = async (cwd: string, command: string, args: string[]) => {
  const output = await new Deno.Command(command, { cwd, args, stdout: 'piped', stderr: 'piped' }).output();
  const stdout = decoder.decode(output.stdout).trim();
  if (!output.success) throw new Error(`${command} failed: ${decoder.decode(output.stderr) || stdout}`);
  return stdout;
};
...
Deno.test('server snapshots reject likely secret files', async () => {
  ...
  try {
    ...
    assert(message.includes('private.key'), `Expected secret rejection, got: ${message}`);
  } finally {
    await Deno.remove(root, { recursive: true });
  }
});
...
Deno.test('server snapshots reject unresolved merge conflicts', async () => {
  ...
  try {
    ...
    await Deno.writeTextFile(`${root}/server/app.ts`, 'export const value = "other";\n');
    await git(root, 'commit', '-qam', 'other change');
    await git(root, 'checkout', '-q', '-');
    await Deno.writeTextFile(`${root}/server/app.ts`, 'export const value = "current";\n');
    ...
    assert(message.includes('Resolve merge conflicts'), `Expected conflict rejection, got: ${message}`);
  } finally {
    await Deno.remove(root, { recursive: true });
  }
});
...
Deno.test('deployment ref push rejects a stale force-with-lease expectation', async () => {
  ...
  try {
    ...
    await pushRefAtLease(root, 'test', first.commit, 'refs/heads/deploy/pi-dev', undefined);

    await Deno.writeTextFile(`${root}/server/app.ts`, 'export const value = 3;\n');
    const second = await createServerSnapshot(root);
    let message = '';
    try {
      await pushRefAtLease(root, 'test', second.commit, 'refs/heads/deploy/pi-dev', first.parent);
    } catch (error) {
      message = error instanceof Error ? error.message : String(error);
    }
    assert(message.length > 0, 'stale force-with-lease unexpectedly succeeded');
    assertEquals(
      await git(root, 'ls-remote', '--heads', 'test', 'refs/heads/deploy/pi-dev').then((line) => line.split(/\s+/)[0]),
      first.commit,
    );
  } finally {
    await Deno.remove(root, { recursive: true });
...
const testDokploy = (terminalStatus: 'done' | 'error' | 'cancelled' | 'running' | 'token-error') => {
  let triggered = false;
  let polls = 0;
  let authFailures = 0;
  const server = Deno.serve({ hostname: '127.0.0.1', port: 0, onListen: () => undefined }, (request) => {
    ...
    if (url.pathname === '/api/deployment.allByCompose') {
      ...
      return Response.json([{
        ...
        status: polls === 1 ? 'queued' : polls === 2 || terminalStatus === 'token-error' ? 'running' : terminalStatus,
        ...(terminalStatus === 'error' ? { errorMessage: 'build failed' } : {}),
      }]);
    }
    if (url.pathname === '/api/deployment.readLogs') return Response.json({ logs: 'remote build log' });
...
Deno.test('Dokploy client waits for a successful deployment with API-key auth', async () => {
  ...
  try {
    const deployment = await triggerAndWaitForDeployment(fake.config, 'test deployment', 'test');
...
Deno.test('Dokploy client includes remote logs when deployment fails', async () => {
  ...
  try {
    ...
    assert(message.includes('build failed'), `Expected deployment error, got: ${message}`);
    assert(message.includes('remote build log'), `Expected deployment logs, got: ${message}`);
  } finally {
    await fake.server.shutdown();
  }
});
...
Deno.test('Dokploy client reports cancelled deployments with logs', async () => {
  ...
  try {
    ...
    assert(message.includes('cancelled'), `Expected cancellation, got: ${message}`);
    assert(message.includes('remote build log'), `Expected deployment logs, got: ${message}`);
  } finally {
    await fake.server.shutdown();
  }
...
Deno.test('Dokploy client redacts its API token from error responses', async () => {
  ...
  try {
    ...
    assert(message.includes('[REDACTED]'), `Expected redacted token marker, got: ${message}`);
    assert(!message.includes('test-token'), `API token leaked in error: ${message}`);
  } finally {
    await fake.server.shutdown();
  }
});
