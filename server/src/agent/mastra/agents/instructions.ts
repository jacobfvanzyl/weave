...
export const gitProjectCodingInstructions = [
  ...
  '- After enough discovery to know the intended file/module changes, use write_proposal or write_proposal_patch to create .agents/proposals/*.md before any write/edit tool call against source files.',
  ...
  '- Use write_proposal to create .agents/proposals/*.md for small Guided proposals, and use write_proposal_patch for larger proposal regeneration from a patch file.',
  '- Each file_edit proposal item must contain a real unified diff that applies to the current repository file. Do not provide currentContent for new file_edit items; the tool computes current_hash from the repository.',
  '- Each file_delete proposal item should provide only the path; the tool computes current_hash from the repository. Each file_create item must provide proposedContent or a create-style unified diff, and the path must not already exist.',
  ...
  '- For larger proposal rewrites, write a multi-file unified patch under a workspace scratch path, then call write_proposal_patch with patchPath plus one metadata entry per file. Do not print the patch or read it back into chat just to assemble tool JSON.',
  '- Do not build large single-line JSON proposal payloads through shell/read loops. Keep large diffs in patch files and pass paths to tools.',
  '- Put optional prose in description or rationale, never in diff or proposedContent. A diff must be executable patch content, not natural-language implementation notes.',
  ...
  '- Before implementing approved proposal items, verify every approved code item is complete in the artifact body: file_edit must have a Unified Diff with a matching current_hash, file_create must have Proposed Content or a create-style Unified Diff, and file_delete must have current_hash. Legacy content-block proposals remain valid when their hashes match the corresponding blocks.',
  ...
  '- Do not implement source changes unless all code proposal items are approved. If any item has requested changes, revise the proposal to address feedback instead of editing source files.',
  ...
  '- After implementation and validation, use update_proposal to mark implemented approved items as applied, or record stale/changes_requested outcomes.',
