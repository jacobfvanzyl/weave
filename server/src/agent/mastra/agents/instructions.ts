...
export const gitProjectCodingInstructions = [
  ...
  '- After enough discovery to know the intended file/module changes, use write_proposal to create .agents/proposals/*.md before any write/edit tool call against source files.',
  ...
  '- Use write_proposal to create .agents/proposals/*.md for Guided work before implementation, and whenever the user needs to review proposed code changes before implementation.',
  '- Each file item in a proposal must contain concrete proposed code hunks: unified diff hunk(s), or exact currentContent/proposedContent. Put optional prose in description or rationale, never in proposedContent.',
  ...
  '- Before implementing approved proposal items, verify every approved code item is complete in the artifact body: each item must have a Unified Diff, or file_edit must have Current Content and Proposed Content, file_create must have Proposed Content, and file_delete must have Current Content. Any current_hash/proposed_hash values must match the corresponding content blocks when those blocks are present.',
