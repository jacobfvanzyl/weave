...
import type { CompactionMode, StructuredCheckpointV2 } from './thread-compaction';

export type ThreadCompactionTrigger = 'manual' | 'automatic';
export type ThreadCompactionStatus = 'running' | 'completed' | 'rejected' | 'failed' | 'cancelled';
...
export type ThreadCompactionRecord = {
  ...
  instructions?: string;
  summary?: string;
  checkpoint?: StructuredCheckpointV2;
  ...
  projectionBeforeTokens?: number;
  projectionAfterTokens?: number;
...
const recordFromRow = (row: Record<string, unknown>): ThreadCompactionRecord => ({
  ...
  checkpoint: optionalJson<StructuredCheckpointV2>(row.checkpoint_json),
  compactionMode: optionalString(row.compaction_mode) as CompactionMode | undefined,
  ...
  projectionBeforeTokens: optionalNumber(row.projection_before_tokens),
  projectionAfterTokens: optionalNumber(row.projection_after_tokens),
  reclaimedTokens: optionalNumber(row.reclaimed_tokens),
...
const selectColumns = `
  ...
  checkpoint_json, compaction_mode,
  ...
  source_tokens, summary_tokens, projected_tokens, projection_before_tokens, projection_after_tokens,
  reclaimed_tokens, decision_reason, source_fingerprint, error,
...
export class ThreadCompactionRepository {
    ...
    try {
      await db.execute({
        sql: `insert into thread_compactions (
          ...
          source_tokens, compaction_mode, started_at, created_at, updated_at
        ) values (?, ?, ?, ?, ?, ?, 'running', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [
          id,
          input.resourceId,
  ...
  async complete(id: string, result: {
    ...
    const db = await this.getDb();
    await db.execute({
      sql: `update thread_compactions set status = 'completed', summary = ?, checkpoint_json = ?::jsonb,
        compacted_through_message_id = ?,
        compacted_through_created_at = ?, compacted_message_count = ?, first_retained_message_id = ?,
        source_tokens = ?, summary_tokens = ?, projected_tokens = ?, projection_before_tokens = ?,
        projection_after_tokens = ?, reclaimed_tokens = ?, decision_reason = null, source_fingerprint = ?,
      ...
      args: [
        ...
        result.projectionBeforeTokens,
        result.projectionAfterTokens,
        result.reclaimedTokens,
  ...
  async reject(id: string, result: {
    ...
    const db = await this.getDb();
    await db.execute({
      sql: `update thread_compactions set status = 'rejected', decision_reason = ?,
        projection_before_tokens = ?, projection_after_tokens = ?, reclaimed_tokens = ?,
        projected_tokens = ?, completed_at = now(), updated_at = now()
