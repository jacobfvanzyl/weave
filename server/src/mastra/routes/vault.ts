...
export const vaultRoutes = [
  ...
  defineRoute('/vault/mkdir', {
    method: 'POST',
    handler: async c => handleVaultRoute(c, 'mkdir', body => ({
      path: optionalString(body.path) ?? '',
    })),
  }),
  defineRoute('/vault/move', {
    method: 'POST',
