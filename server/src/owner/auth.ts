...
export const loadOwnerAuthConfig = (env: Record<string, string | undefined> = process.env): OwnerAuthConfig => {
  ...
  return {
    token,
    owner: {
      id: optionalString(env.WEAVE_OWNER_ID) ?? optionalString(env.WEAVE_AUTH_USER_ID) ?? legacyOwner?.ownerId ?? 'local-user',
      name: optionalString(env.WEAVE_OWNER_NAME) ?? optionalString(env.WEAVE_AUTH_USER_NAME) ?? legacyOwner?.ownerName ?? 'Local User',
      role: 'owner',
    },
  };
};
