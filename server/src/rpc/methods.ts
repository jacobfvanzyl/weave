...
} from "../modules/notes/storage/resolver.ts";
import type {
  NotesProject,
  NotesVaultBackend,
  ResolvedNotesVaultBinding,
} from "../modules/notes/storage/types.ts";
import {
  adjustRpcSubscriptions,
  recordRpcReplayEvent,
  recordRpcReplayRequest,
} from "./metrics.ts";
...
export const registerCoreRpcMethods = (
  ...
  router.register("binary.ack", ["client", "portal"], () => ({ ok: true }));
  router.register(
    "binary.abort",
    ["client", "portal"],
  ...
  router.register("owner.get", "client", (_params, { session }) => {
      ...
      guarded(async () => {
        ...
        await services.agent.service.reorderChatThreads({
          ...
          threadIds: Array.isArray(body.threadIds)
            ? body.threadIds.filter((id): id is string =>
              typeof id === "string"
            )
            : [],
          scope: {
  ...
  router.register(
    ...
    (params, { session }) =>
      guarded(async () => ({
        messages: await services.agent.service.getChatThreadRawMessages({
          resourceId: session.ownerContext.owner.id,
  ...
  router.register(
    ...
    (params, { session }) =>
      guarded(() => {
        const body = recordParams(params);
        return services.agent.service.getChatThreadContextUsage({
          resourceId: session.ownerContext.owner.id,
  ...
  router.register(
    ...
    (params, { session }) =>
      guarded(async () => {
        ...
        const abort = () =>
          void reader.cancel("RPC connection closed.").catch(() => undefined);
  ...
  router.register(
    ...
    (params, { session }) =>
      guarded(async () => {
        ...
        chatSubscriptions.delete(subscriptionId);
        await subscription?.reader.cancel("Unsubscribed.").catch(() =>
          undefined
        );
        return { ok: true };
      }),
  ...
  const artifactName = (params: unknown) =>
    requiredString(recordParams(params).name, "name");
  ...
  router.register(
    ...
    (_params, { session }) =>
      guarded(async () => ({
        prompts:
          (await userArtifactRepository.listPrompts(artifactOwner(session)))
            .map(artifactSummary),
      })),
  );
  router.register(
  ...
  router.register(
    ...
    (_params, { session }) =>
      guarded(async () => ({
        skills:
          (await userArtifactRepository.listSkills(artifactOwner(session))).map(
            artifactSummary,
          ),
      })),
  );
      ...
      guarded(async () => ({
        workflow: workflowDefinitionResponse(
          await workflowControlService.getDefinition(
            session.ownerContext.owner.id,
            requiredString(recordParams(params).workflowId, "workflowId"),
          ),
        ),
      })),
  );
    ...
    (params, { session }) =>
      guarded(async () => {
        ...
        const run = await workflowControlService.startRun({
          ownerId: session.ownerContext.owner.id,
          workflowId: requiredString(body.workflowId, "workflowId"),
  ...
  router.register(
    ...
    (params, { session }) =>
      guarded(async () => {
        const body = recordParams(params);
        const limit = Number.isSafeInteger(body.limit) && Number(body.limit) > 0
          ? Number(body.limit)
          : undefined;
        return {
    ...
    (params, { session }) =>
      guarded(async () => {
        const run = await workflowControlService.getRun(
          session.ownerContext.owner.id,
          requiredString(recordParams(params).runId, "runId"),
        );
        if (!run) {
          throw new RpcApplicationError(
            rpcErrorCode.notFound,
  ...
  router.register("portal.browse", "client", (params, { session }) => {
    const body = recordParams(params);
    return guarded(() =>
      browseOwnerPortal({
        ownerId: session.ownerContext.owner.id,
  ...
  router.register(
    ...
    "client",
    (_params, { session }) =>
      guarded(() => issueOwnerPortalToken(session.ownerContext.owner.id)),
  );
  ...
  const callPortalSession = async (input: {
    session: RpcSession;
    params: unknown;
    ...
    const clientId =
      `${input.session.connectionId}:${input.channel}:${sessionId}`;
  ...
  const terminalCall = (action: string, params: unknown, session: RpcSession) =>
    guarded(() =>
      callPortalSession({
        session,
        params,
  ...
  const invokeNotesBackend = async (
    ...
    if (action === "upload") {
      return await backend.upload(binding, {
        path: optionalString(body.path) ?? "",
        base64Content: typeof body.base64Content === "string"
          ? body.base64Content
          : undefined,
      ...
      (params, { session }) =>
        guarded(async () => {
          ...
          if (action === "read" && remoteTransfer) {
            const descriptor = remoteTransfer as BinaryTransferDescriptor;
  ...
  const lspCall = (action: string, params: unknown, session: RpcSession) =>
    guarded(() =>
      callPortalSession({
        session,
        params,
  ...
  const jupyterCall = (action: string, params: unknown, session: RpcSession) =>
    guarded(() =>
      callPortalSession({
        session,
        params,
  ...
  router.register("client.surface.update", "client", (params, { session }) => {
    if (session.role !== "client") {
      throw new Error("Expected a client RPC session.");
    }
    const record = isRecord(params) ? params : {};
    return updateClientToolHost(
      session.clientId,
      normalizeClientToolHello(record),
    ) ?? null;
  });
};
