...
export type TerminalHostEvent =
  | { type: 'started'; terminalId: string; demiplaneId?: string; sessionId: string; cwd: string; pid?: number; cols: number; rows: number }
...
const sanitizeTerminalClientMessage = (message: Record<string, unknown>, token: TerminalTokenRecord): TerminalClientMessage => {
  if (message.type === 'start') {
    ...
    return {
      ...
      planeId: token.planeId,
      demiplaneId: token.demiplaneId,
      rootId: token.rootId,
