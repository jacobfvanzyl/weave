...
function CardFrame({ className, render, ...props }: useRender.ComponentProps<"div">) {
  const defaultProps = {
    className: cn(
      "relative flex flex-col overflow-hidden rounded-lg border bg-card text-card-foreground divide-y divide-border *:data-[slot=card]:rounded-none *:data-[slot=card]:border-0 *:data-[slot=card]:shadow-none",
      className,
    ),
